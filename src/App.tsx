...
function App() {
    return (
        <div>
            ...
            <div className='main-content'>
                <h1>What is ANOVA?</h1>
                <p>ANOVA (short for Analysis of Variance) is a statistical test used to determine if there is a difference between the means of two or more groups. An example of a question an ANOVA test can answer is "Is there a difference in the price of apples between different grocery store brand?".</p>
                <h1>What are Factors, Levels, and Responses?</h1>
                <p>A factor is a categorical variable (a variable whose value is from a limited set) which is used to try to explain changes in the response. The response is a quantitative variable (a variable whose value is an amount). The different values a factor can be are called levels. From the example above, the factor is the grocery store brand, the levels are the specific grocery store brands used in the experiment (i.e.  Brand A, Brand B, Brand C, ...), and the response is the the price of apples at a grocery store.</p>
